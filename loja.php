<?php
require 'conexao.php';

// Busca os usuários para simular a sessão
$sqlUsers = "SELECT id, nome, xp_acumulado FROM usuarios";
$users = $pdo->query($sqlUsers)->fetchAll();

// Busca o catálogo
$sqlRecompensas = "SELECT * FROM recompensas ORDER BY custo_xp ASC";
$recompensas = $pdo->query($sqlRecompensas)->fetchAll();
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <title>Loja de Recompensas</title>
    <style>
        * { box-sizing: border-box; font-family: 'Segoe UI', Tahoma, sans-serif; }
        body { background-color: #121212; color: #e0e0e0; padding: 20px; }
        .container { max-width: 800px; margin: 0 auto; }
        h2 { border-bottom: 2px solid #FFD700; padding-bottom: 10px; color: #FFD700; }
        .card { background-color: #1e1e1e; padding: 20px; margin-bottom: 15px; border-radius: 8px; border-left: 5px solid #FFD700; display: flex; justify-content: space-between; align-items: center; }
        .info h3 { margin: 0 0 5px 0; }
        .info p { margin: 0; color: #aaa; font-size: 14px; }
        .acao { text-align: right; }
        select, button { padding: 8px; border-radius: 4px; border: none; background-color: #2c2c2c; color: white; margin-top: 5px; }
        button { background-color: #FFD700; color: #000; font-weight: bold; cursor: pointer; }
        button:hover { background-color: #e6c200; }
        .stats { background-color: #2c2c2c; padding: 15px; border-radius: 8px; margin-bottom: 20px; display: flex; gap: 20px; }
        .nav { text-align: center; margin-top: 20px; }
        .nav a { color: #FFD700; text-decoration: none; font-weight: bold; margin: 0 10px; }
    </style>
</head>
<body>
    <div class="container">
        <h2>Seu Caixa de XP</h2>
        <div class="stats">
            <?php foreach ($users as $u): ?>
                <div><strong><?= htmlspecialchars($u['nome']) ?>:</strong> <?= $u['xp_acumulado'] ?> XP</div>
            <?php endforeach; ?>
        </div>

        <h2>Catálogo de Resgate</h2>
        <?php foreach ($recompensas as $r): ?>
            <div class="card">
                <div class="info">
                    <h3><?= htmlspecialchars($r['titulo']) ?></h3>
                    <p>Custo: <?= $r['custo_xp'] ?> XP</p>
                </div>
                <div class="acao">
                    <form action="processa_compra.php" method="POST">
                        <input type="hidden" name="recompensa_id" value="<?= $r['id'] ?>">
                        <input type="hidden" name="custo" value="<?= $r['custo_xp'] ?>">
                        <select name="usuario_id" required>
                            <option value="">Quem vai resgatar?</option>
                            <?php foreach ($users as $u): ?>
                                <option value="<?= $u['id'] ?>"><?= htmlspecialchars($u['nome']) ?> (<?= $u['xp_acumulado'] ?>)</option>
                            <?php endforeach; ?>
                        </select>
                        <br>
                        <button type="submit">Resgatar</button>
                    </form>
                </div>
            </div>
        <?php endforeach; ?>

        <div class="nav">
            <a href="index.php">⬅ Cadastrar Missão</a> | 
            <a href="painel.php">Ir para o Painel ➔</a>
        </div>
    </div>
</body>
</html>